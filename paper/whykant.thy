...
text \<open>Kant's theory is based on the idea of practical reason, which is the kind of reason that we 
...
text \<open>Kantian ethics is natural to formalize because the categorical imperative, and the FUL in particular, 
...
for all the individuals involved. \<close>
...
the debates above will need to be resolved. \<close>

subsection "Virtue Ethics"

subsubsection "What Is Virtue"

text \<open>The virtue ethical tradition generally place the virtues, or those traits that constitute a 
...
progress, whereas Kantian ethics automatically presents a formal rule to implement. \<close>
